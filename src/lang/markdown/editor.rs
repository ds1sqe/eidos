...
use egui::{
    Key, KeyboardShortcut, Modifiers, ScrollArea, TextBuffer, TextEdit, Ui,
    text::CCursorRange,
};
...
impl Editor {
            ...
            ui.columns(2, |columns| {
                ...
                ScrollArea::vertical().id_salt("rendered").show(
                    ...
                    |ui| {
                        ...
                        super::render::render(ui, &self.code);
                    },
                );
            });
        } else {
    ...
    fn editor_ui(&mut self, ui: &mut egui::Ui) {
        ...
        let response = if self.highlight_editor {
            let mut layouter =
                |ui: &egui::Ui, easymark: &dyn TextBuffer, wrap_width: f32| {
                    let mut layout_job =
                        highlighter.highlight(ui.style(), easymark.as_str());
...
pub const SHORTCUT_BOLD: KeyboardShortcut =
    KeyboardShortcut::new(Modifiers::COMMAND, Key::B);
pub const SHORTCUT_CODE: KeyboardShortcut =
...
fn toggle_surrounding(
    ...
    let prefix_crange =
        primary.index.saturating_sub(surrounding_ccount)..primary.index;
    let suffix_crange =
        secondary.index..secondary.index.saturating_add(surrounding_ccount);
    let already_surrounded = code.char_range(prefix_crange.clone())
        == surrounding
